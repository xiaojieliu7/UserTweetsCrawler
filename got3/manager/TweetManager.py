...
class TweetManager:
    ...
    @staticmethod
    def getAgricultureTweets(category, tweetCriteria, receiveBuffer=None, bufferLength=100, proxy=None, mongourl='101.132.187.45:27017'):
        refreshCursor = ''
        client = pymongo.MongoClient(mongourl)
        db = client.get_database('train_data')
        collection = db.get_collection('agriculture_log')
        resultlog = collection.find_one({'until': tweetCriteria.until})
        if resultlog is not None:
            refreshCursor = resultlog['refreshCursor']
